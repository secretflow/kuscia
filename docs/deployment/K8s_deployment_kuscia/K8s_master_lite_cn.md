...
## 前言
本教程帮助你在 k8s 集群上使用 [中心化组网模式](../../reference/architecture_cn.md#中心化组网模式) 来部署 Kuscia 集群。目前 kuscia 在部署到 k8s 上时，隐私计算任务的运行态仅支持 runk 模式，runp 模式正在开发中，详细参考[容器运行模式](../../reference/architecture_cn.md#agent)；runc 模式目前需要部署 kuscia 的 pod 有特权容器，暂时不是特别推荐，所以下文默认的模式以 runk 模式来进行部署（需要能够有权限在宿主的 K8s 上拉起任务 pod）。
