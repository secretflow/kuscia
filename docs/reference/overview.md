...
## Kuscia是什么
隐语是从蚂蚁集团内部使用隐私计算技术助力业务发展的经验中沉底而来的。在过往的隐私计算技术落地实际业务过程中共经历了三个时期：手动拉起任务时期、Coordinator 发起任务时期、Kuscia 编排任务时期。
![Kuscia Evolution](../imgs/kuscia_evolution.png)
### 手动拉起任务时期
...
在工业化落地隐私计算场景下，大部分隐私计算平台可以简化为三层去理解，分别是平台层、调度层、引擎层。对应到隐语的开源体系分别是： SecretPad、Kuscia 、SecretFlow/TrustedFlow/SCQL 。
- 平台层：平台层一般为图形化界面应用，是直接面向终端用户的产品，用户可通过可视化界面的操作完成隐私求交、隐私保护机器学习、隐私保护数据分析等任务。无需感知底层系统的实现细节。隐语开源的 SecretPad 即位于平台层。平台层可通过 Kuscia 层提供的 KusciaAPI 执行隐私计算任务。
- Kuscia 层：聚焦于解决隐私计算技术工业化生产落地可能遇到的共性问题（如跨域的网络互通、任务编排、异构数据源、多引擎集成、互联互通等问题）。
- 引擎层：完成核心的隐私计算任务，如 PSI 求交，PIR，隐私保护的特征处理、LR XGB 模型训练、模型预测等。

![Kuscia Layer](../imgs/kuscia_layer.png)
...
- 服务发现：为保证任务之间通信的便捷性，Kuscia 会自动为引擎提供各方的通讯地址，消除了手动配置网关、验证链路安全或确认对方身份的繁琐过程。引擎可以直接使用这些地址安全地与其他方通信。使隐私计算引擎像使用本地网络一样使用跨域网络。
- 网络拓扑多样化：在隐私计算场景中，网络拓扑千差万别。在机构端口暴露问题上，Kuscia 可做到多任务并发运行时复用一个公网端口（一些条件下甚至不暴露公网端口），在机构间通讯链路上，支持机构间直连（无法直连时）跨机构流量转发等能力。
...
- BFIA（北京金融科技产业联盟数据专委会） 互联互通标准：Kuscia 目前已支持 [BFIA 标准的调度面协议](../tutorial/run_bfia_job_cn.md)，可快速与其他已支持 BFIA 标准的机构互通。
