...
Kuscia（Kubernetes-based Secure Collaborative InfrA）是一款基于 K8s 的隐私计算应用编排框架，旨在屏蔽异构基础设施和协议，并提供统一的隐私计算底座。在此基础上，kuscia 提供了资源管理、应用调度、容器加载、服务发现、数据安全访问、运维监控等诸多能力。
...
Kuscia 支持 Lite 节点与 Autonomy 节点、以及两个中心化网络互联互通，并计划支持与第三方产商的节点互联互通，从而构建更大的隐私计算网络。
...
控制平面监听和响应集群事件，实现资源管理和应用调度功能，核心组件包括 K3s、Kuscia Controllers、Kuscia Storage、Envoy、互联互通适配器。
...
- DomainRoute Controller：路由控制器，负责管理节点与节点、节点与 Master、Master 之间的路由规则以及身份认证和鉴权策略。
...
- Interop Controller：点对点控制器，主要负责不同 Kuscia 控制平面下的节点进行互通，从而协同完成多方任务调度。

#### Kuscia Storage

Kuscia Storage 是对 Kubernetes 原生集群存储的补充。Kubernetes 原生集群存储不适合存储大 value，因此对于大 value 的资源属性，如作业配置等，将存储在 Kuscia Storage 中。

#### 互联互通适配器

互联互通适配器主要用在与其他厂商进行互联互通时，对 DAG 和调度接口的适配。
...
Lite 节点主要由 Agent、CoreDNS、Envoy、Data Manager（暂未实现）、Data Proxy（暂未实现）组成，提供容器管理、节点路由、身份认证与鉴权、数据管理等功能。
...
Data Manager 主要用于数据源和数据集（数据表、模型、任务报告等）的注册和管理，提供元信息增删改查等功能。

#### Data Proxy

Data Proxy 主要用于数据代理，为应用屏蔽异构数据源的差异，提供统一的读写接口，支持数据审计、并行传输、断点续传等功能。
...
在该模式下，参与方通过 Interop Controller，
...
在 Kuscia 编排框架中，作业（Job）编排和任务（Task）调度主要通过 Job Controller、Task Controller、Interop Controller、Kuscia Scheduler 协同完成。
...
- Interop Controller：用于点对点（P2P）组网模式下，调度方与参与方之间的 Task 资源同步。
