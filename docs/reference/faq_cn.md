...
### WSL 启动 Docker
**WSL 中想要使用 Docker，必须安装 docker desktop**，具体安装步骤如下：
...
```
需要在非只读目录(如 D 盘)下创建 Docker Desktop 默认安装路径(如 D：\Docker)，然后以管理员身份运行 cmd，通过以下命令创建链接:
```shell
...
Docker 镜像默认保存在C盘， FATE 作业镜像很大，可能导致C盘空间不足，因此建议改为其他空间足够的磁盘。具体做法是，打开 Docker Desktop，找到 Settings->Resources->Browse，修改镜像保存路径。
