...
func (m *kubeGenericRuntimeManager) Version() (pkgcontainer.Version, error) {
	typedVersion, err := m.getTypedVersion()
	if err != nil {
		return nil, err
	}
...
func (m *kubeGenericRuntimeManager) SyncPod(pod *v1.Pod, podStatus *pkgcontainer.PodStatus, auth *credentialprovider.AuthConfig, backOff *flowcontrol.Backoff) (result pkgcontainer.PodSyncResult) {
	...
	if podContainerChanges.KillPod {
		...
		killResult := m.killPodWithSyncResult(pod, pkgcontainer.ConvertPodStatusToRunningPod(m.runtimeName, podStatus), nil)
	...
	start := func(typeName, metricLabel string, spec *startSpec) error {
		...
		if msg, err := m.startContainer(podSandboxID, podSandboxConfig, spec, pod, podStatus, auth, podIP, podIPs); err != nil {
	...
	for _, idx := range podContainerChanges.ContainersToStart {
		_ = start("container", metrics.Container, containerStartSpec(&pod.Spec.Containers[idx]))
	}
	return
}

// killContainersWithSyncResult kills all pod's containers with sync results.
func (m *kubeGenericRuntimeManager) killContainersWithSyncResult(pod *v1.Pod, runningPod pkgcontainer.Pod, gracePeriodOverride *int64) (syncResults []*pkgcontainer.SyncResult) {
...
func (m *kubeGenericRuntimeManager) killPodWithSyncResult(pod *v1.Pod, runningPod pkgcontainer.Pod, gracePeriodOverride *int64) (result pkgcontainer.PodSyncResult) {
...
func (m *kubeGenericRuntimeManager) KillPod(pod *v1.Pod, runningPod pkgcontainer.Pod, gracePeriodOverride *int64) error {
...
func (m *kubeGenericRuntimeManager) GetPodStatus(uid kubetypes.UID, name, namespace string) (*pkgcontainer.PodStatus, error) {
...
func (m *kubeGenericRuntimeManager) GetPods(all bool) ([]*pkgcontainer.Pod, error) {
	pods := make(map[kubetypes.UID]*pkgcontainer.Pod)
	sandboxes, err := m.getPodSandboxes(all)
	if err != nil {
		return nil, err
	}
...
func (m *kubeGenericRuntimeManager) GarbageCollect(gcPolicy pkgcontainer.GCPolicy, allSourcesReady bool, evictNonDeletedPods bool) error {
	return m.containerGC.GarbageCollect(gcPolicy, m.podStdoutRootDirectory, allSourcesReady, evictNonDeletedPods)
}
