...
func TestPullImage(t *testing.T) {
	...
	imageRef, err := fakeManager.PullImage(pkgcontainer.ImageSpec{Image: "busybox"}, nil, nil)
	assert.NoError(t, err)
...
func TestGetImageRef(t *testing.T) {
	...
	imageRef, err := fakeManager.GetImageRef(pkgcontainer.ImageSpec{Image: image})
	assert.NoError(t, err)
...
func TestRemoveImage(t *testing.T) {
	...
	_, err = fakeManager.PullImage(pkgcontainer.ImageSpec{Image: "busybox"}, nil, nil)
	assert.NoError(t, err)
	assert.Equal(t, 1, len(fakeImageService.Images))

	err = fakeManager.RemoveImage(pkgcontainer.ImageSpec{Image: "busybox"})
	assert.NoError(t, err)
...
func TestPullWithSecrets(t *testing.T) {
	...
	for description, test := range tests {
		...
		_, err = fakeManager.PullImage(pkgcontainer.ImageSpec{Image: test.imageName}, test.auth, nil)
		require.NoError(t, err)
