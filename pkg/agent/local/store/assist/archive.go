...
func Untar(dst string, isGzip bool, adjustLink bool, r io.Reader) error {
	var tr *tar.Reader
	if isGzip {
	...
	for {
		...
		if err := WriteOneFile(header, tr, dst, target, adjustLink); err != nil {
			return err
		}
	}
}

func ExtractTar(dst string, tarFile io.ReadSeeker, adjustLink bool) error {
	isGzip := false

	header := make([]byte, 2)
	_, err := tarFile.Read(header)
	if err != nil {
		return err
	}
	...
	return Untar(dst, isGzip, adjustLink, tarFile)
}
...
func ExtractTarFile(dst string, tarFile string, adjustLink bool) error {
	...
	return ExtractTar(dst, file, adjustLink)
}

func WriteOneFile(header *tar.Header, tr *tar.Reader, targetWd, target string, adjustLink bool) error {
	...
	default:
		return fmt.Errorf("unsupported file type, file-> %+v", header)
	}
	return nil
}
