...
func AddMasterClusters(ctx context.Context, namespace string, config *MasterConfig) error {
	if !config.Master {
		masterProxyCluster, err := generateMasterCluster(serviceMasterProxy, config.MasterProxy)
		if err != nil {
...
func addMasterCluster(service, namespace string, config *ClusterConfig) error {
	localCluster, err := generateMasterCluster(service, config)
	if err != nil {
		...
		virtualHost.Routes[0].Match.PathSpecifier = &route.RouteMatch_SafeRegex{
			SafeRegex: &matcherv3.RegexMatcher{
				Regex: "/(api(s)?(/[0-9A-Za-z_.-]+)?/v1(alpha1)?/namespaces/[0-9A-Za-z_.-]+" +
					"/(pods|gateways|domainroutes|endpoints|services|events|configmaps|leases|taskresources|secrets)" +
...
func generateMasterCluster(name string, config *ClusterConfig) (*envoycluster.Cluster, error) {
