...
func TestLoadMetricConfig(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			expectedMetrics: []string{"recvbytes", "xmitbytes", "recvbw", "xmitbw", "cpu_percentage", "cpu_usage", "memory", "disk", "inodes", "virtual_memory", "physical_memory", "readbytes", "writebytes"},
			expectedAgg: map[string]string{
				"recvbytes":       "sum",
				"xmitbytes":       "sum",
				"recvbw":          "sum",
