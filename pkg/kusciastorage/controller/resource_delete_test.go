...
func TestValidateForResourceDelete(t *testing.T) {
	h := NewResourceDeleteHandler()
	testCases := []struct {
		name        string
		ctx         *api.BizContext
...
func TestHandleForResourceDelete(t *testing.T) {
	...
	testCases := []struct {
		...
		ctx         *api.BizContext
		params      map[string]string
		h           *resourceDeleteHandler
		request     *api.AnyStringProto
		expectedErr bool
	}{
		{
			name: "delete resource failed",
			ctx: &api.BizContext{
