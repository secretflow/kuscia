...
func (c *hostResourcesController) syncPodHandler(ctx context.Context, key string) error {
	...
	hExtractedPodSpec := utilscommon.ExtractPodSpec(hPodCopy)
	...
	mExtractedPodSpec := utilscommon.ExtractPodSpec(mPodCopy)
	if err = utilscommon.PatchPod(ctx, c.memberKubeClient, mExtractedPodSpec, hExtractedPodSpec); err != nil {
