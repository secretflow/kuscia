...
func (c *hostResourcesController) syncTaskResourceHandler(ctx context.Context, key string) error {
	...
	hExtractedTr := utilscommon.ExtractTaskResource(hTrCopy)
	...
	mExtractedTr := utilscommon.ExtractTaskResource(mTrCopy)
	if err = utilscommon.PatchTaskResource(ctx, c.memberKusciaClient, mExtractedTr, hExtractedTr); err != nil {
