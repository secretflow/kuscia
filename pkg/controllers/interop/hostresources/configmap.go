...
func (c *hostResourcesController) syncConfigMapHandler(ctx context.Context, key string) error {
	...
	hExtractedConfigMap := utilscommon.ExtractConfigMap(hConfigMapCopy)
	...
	mExtractedConfigMap := utilscommon.ExtractConfigMap(mConfigMapCopy)
	if err = utilscommon.PatchConfigMap(ctx, c.memberKubeClient, mExtractedConfigMap, hExtractedConfigMap); err != nil {
