...
// CreatingHandler is used to handle kuscia task which phase is creating.
...
func (h *CreatingHandler) Handle(kusciaTask *kusciaapisv1alpha1.KusciaTask) (needUpdate bool, retErr error) {
	createdCondition := getKusciaTaskCondition(&kusciaTask.Status, kusciaapisv1alpha1.ResourceCreated)
	defer func() {
		now := metav1.Now().Rfc3339Copy()
		createdCondition.LastTransitionTime = now
		if retErr == nil {
			createdCondition.Status = v1.ConditionTrue
		} else {
			createdCondition.Status = v1.ConditionFalse
			createdCondition.Reason = "KusciaTaskCreateFailed"
			createdCondition.Message = "Failed: " + retErr.Error()
		}
	}()

	// create pod and svc
	if err := h.createTaskResources(kusciaTask); err != nil {
		return false, fmt.Errorf("failed to create kusciaTask %q resource, %v", kusciaTask.Name, err)
	}

	kusciaTask.Status.Phase = kusciaapisv1alpha1.TaskRunning
	kusciaTask.Status.Reason = "KusciaTaskCreated"
	kusciaTask.Status.Message = "All resource created successfully"

	return true, nil
}

func (h *CreatingHandler) createTaskResources(kusciaTask *kusciaapisv1alpha1.KusciaTask) error {
...
func (h *CreatingHandler) buildPartyKitInfo(kusciaTask *kusciaapisv1alpha1.KusciaTask, party *kusciaapisv1alpha1.PartyInfo) (*PartyKitInfo, error) {
...
func (h *CreatingHandler) createResourceForParty(partyKit *PartyKitInfo) (map[string]*kusciaapisv1alpha1.PodStatus, error) {
...
func (h *CreatingHandler) createTaskResourceGroup(kusciaTask *kusciaapisv1alpha1.KusciaTask, partyKitInfos map[string]*PartyKitInfo) error {
	trg := generateTaskResourceGroup(kusciaTask, partyKitInfos)
	return h.submitTaskResourceGroup(trg)
}

func generateTaskResourceGroup(kusciaTask *kusciaapisv1alpha1.KusciaTask, partyKitInfos map[string]*PartyKitInfo) *kusciaapisv1alpha1.TaskResourceGroup {
	var (
	...
	for _, partyKitInfo := range partyKitInfos {
		trgPartyPods := make([]kusciaapisv1alpha1.TaskResourceGroupPartyPod, 0, len(partyKitInfo.pods))
	...
	trg := &kusciaapisv1alpha1.TaskResourceGroup{
		ObjectMeta: metav1.ObjectMeta{
			...
			Labels: map[string]string{
				...
				labelKusciaTaskUID:     string(kusciaTask.UID),
			},
...
func generateConfigMap(partyKit *PartyKitInfo) *v1.ConfigMap {
	labels := map[string]string{
		...
		labelKusciaTaskUID:        string(partyKit.kusciaTask.UID),
	}
...
func (h *CreatingHandler) submitConfigMap(cm *v1.ConfigMap, kusciaTask *kusciaapisv1alpha1.KusciaTask) error {
...
func generatePod(partyKit *PartyKitInfo, podKit *PodKitInfo) (*v1.Pod, error) {
	labels := map[string]string{
		common.LabelController:               KusciaTaskLabelValue,
	...
	pod := &v1.Pod{
		...
		Spec: v1.PodSpec{
			...
			SchedulerName:                kusciaSchedulerName,
...
func (h *CreatingHandler) submitPod(partyKit *PartyKitInfo, pod *v1.Pod) (*v1.Pod, error) {
...
func (h *CreatingHandler) submitService(service *v1.Service, pod *v1.Pod) error {
...
func (h *CreatingHandler) submitTaskResourceGroup(trg *kusciaapisv1alpha1.TaskResourceGroup) error {
