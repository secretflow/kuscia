...
FROM --platform=linux/${ARCH}  ${PROM_NODE_EXPORTER} as node_exporter
FROM --platform=linux/${ARCH}  ${KUSCIA_ENVOY_IMAGE} as kuscia_envoy
...
COPY build/pause/pause-${ARCH}.tar ${ROOT_DIR}/pause/pause.tar
