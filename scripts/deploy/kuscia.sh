...
function start_kuscia_container() {
  local domain_type=$1
  local domain_id=${2:-$DOMAIN_ID}
  local runtime=$3
  local master_endpoint=$4
  local domain_host_port=${5:-$DOMAIN_HOST_PORT}
  local kusciaapi_http_port=${6:-$KUSCIAAPI_HTTP_PORT}
  local kusciaapi_grpc_port=${7:-$KUSCIAAPI_GRPC_PORT}
  local domain_host_internal_port=${8:-$DOMAIN_HOST_INTERNAL_PORT}
  local domain_ctr=$9
  local init_kuscia_conf_file_true_or_false=${10}
  local mount_flag=${11}
  local env_flag=$(generate_env_flag)

  if [[ ${init_kuscia_conf_file_true_or_false} = "true" ]]; then
  ...
  if need_start_docker_container "$domain_ctr"; then
    ...
    start_container "${domain_ctr}" "${domain_id}" "${domain_host_port}" "${kusciaapi_http_port}" "${kusciaapi_grpc_port}" "${domain_host_internal_port}" "${env_flag}" "${kuscia_conf_file}" "${mount_flag}"
    if [[ "$domain_type" != "lite" ]]; then
...
function start_kuscia() {
  ...
  [[ ${DOMAIN_HOST_INTERNAL_PORT} == "" ]] && DOMAIN_HOST_INTERNAL_PORT=13081
  [[ ${KUSCIAAPI_HTTP_PORT} == "" ]] && KUSCIAAPI_HTTP_PORT=13082
  [[ ${KUSCIAAPI_GRPC_PORT} == "" ]] && KUSCIAAPI_GRPC_PORT=13083

  init ${domain_ctr}
  local mount_flag=$(generate_mount_flag)
  start_kuscia_container "${deploy_mode}" "${domain_id}" "$runtime" "$master_endpoint" "${DOMAIN_HOST_PORT}" "${KUSCIAAPI_HTTP_PORT}" "${KUSCIAAPI_GRPC_PORT}" "${DOMAIN_HOST_INTERNAL_PORT}" "${domain_ctr}" "false" "${mount_flag}"
}
...
function start_center_cluster() {
  ...
  start_kuscia_container "master" "${MASTER_DOMAIN}" "" "" "18081" "18082" "18083" "18084" "${MASTER_CTR}" "true"
  start_kuscia_container "lite" "${ALICE_DOMAIN}" "${runtime}" "https://${MASTER_CTR}:1080" "28081" "28082" "28083" "28084" "${alice_ctr}" "true"
  start_kuscia_container "lite" "${BOB_DOMAIN}" "${runtime}" "https://${MASTER_CTR}:1080" "38081" "38082" "38083" "38084" "${bob_ctr}" "true"
...
function start_p2p_cluster() {
  ...
  start_kuscia_container "autonomy" "${ALICE_DOMAIN}" "${runtime}" " " "11081" "11082" "11083" "11084" "${alice_ctr}" "true"
  start_kuscia_container "autonomy" "${BOB_DOMAIN}" "${runtime}" " " "12081" "12082" "12083" "12084" "${bob_ctr}" "true"
...
function start_cxc_cluster() {
  ...
  start_kuscia_container "master" "${alice_master_domain}" "" "" "18081" "18082" "18083" "18084" "${alice_master_ctr}" "true"
  start_kuscia_container "master" "${bob_master_domain}" "" "" "28081" "28082" "28083" "28084" "${bob_master_ctr}" "true"
  start_kuscia_container "lite" "${ALICE_DOMAIN}" "${runtime}" "https://${alice_master_ctr}:1080" "38081" "38082" "38083" "38084" "${alice_ctr}" "true"
  start_kuscia_container "lite" "${BOB_DOMAIN}" "${runtime}" "https://${bob_master_ctr}:1080" "48081" "48082" "48083" "48084" "${bob_ctr}" "true"
...
function start_cxp_cluster() {
  ...
  start_kuscia_container "master" "${alice_master_domain}" "" "" "18081" "18082" "18083" "18084" "${alice_master_ctr}" "true"
  start_kuscia_container "lite" "${ALICE_DOMAIN}" "${runtime}" "https://${alice_master_ctr}:1080" "28081" "28082" "28083" "28084" "${alice_ctr}" "true"
  start_kuscia_container "autonomy" "${BOB_DOMAIN}" "${runtime}" "https://${alice_master_ctr}:1080" "12081" "12082" "12083" "12084" "${bob_ctr}" "true"
